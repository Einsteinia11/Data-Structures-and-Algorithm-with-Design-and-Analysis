...
bool WeightedGraph::addBiDirectionalEdge(int src, int dest, int weight) {
  try {
    ...
    if (addUniDirectionalEdge(src, dest, weight) &&
        addUniDirectionalEdge(dest, src, weight)) {
      return true;
    } else {
...
bool WeightedGraph::getIsWeighted() const { return isWeighted; }
...
int WeightedGraph::getVertexCount() const { return adjList.size() + 1; }
...
bool WeightedGraph::removeEdge(int src, int dest) {
  ...
  if (it != adjList.end()) {
    ...
    auto edgeToRemove = std::remove_if(
        edges.begin(), edges.end(),
        [dest](const std::pair<int, int> &edge) { return edge.first == dest; });
...
int WeightedGraph::getEdgeWeight(int src, int dest) {
  ...
  if (src >= 0 && src < getVertexCount() && dest >= 0 &&
      dest < getVertexCount()) {
